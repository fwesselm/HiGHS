...
void Iterate::Update(double sp, const double* dx, const double* dxl,
  ...
  if (dx) {
    for (Int j = 0; j < n + m; j++)
      if (StateOf(j) != State::fixed) x_[j] += sp * dx[j];
  }
  if (dxl) {
    for (Int j = 0; j < n + m; j++)
      if (has_barrier_lb(j)) {
  ...
  if (dy) {
    for (Int i = 0; i < m; i++) y_[i] += sd * dy[i];
  }
  if (dzl) {
    for (Int j = 0; j < n + m; j++)
      if (has_barrier_lb(j)) {
...
const Vector& Iterate::rb() const {
  Evaluate();
  return rb_;
}
const Vector& Iterate::rl() const {
  Evaluate();
  return rl_;
}
const Vector& Iterate::ru() const {
  Evaluate();
  return ru_;
}
const Vector& Iterate::rc() const {
  Evaluate();
  return rc_;
}
...
double Iterate::pobjective() const {
  Evaluate();
  return pobjective_;
}
double Iterate::dobjective() const {
  Evaluate();
  return dobjective_;
}
...
double Iterate::presidual() const {
  Evaluate();
  return presidual_;
}
double Iterate::dresidual() const {
  Evaluate();
  return dresidual_;
}

double Iterate::complementarity() const {
  Evaluate();
  return complementarity_;
}
double Iterate::mu() const {
  Evaluate();
  return mu_;
}
double Iterate::mu_min() const {
  Evaluate();
  return mu_min_;
}
double Iterate::mu_max() const {
  Evaluate();
  return mu_max_;
}
...
bool Iterate::feasible() const {
  ...
  const bool primal_feasible =
      presidual_ <= feasibility_tol_ * (bounds_measure);
  ...
  if (kTerminationLogging) {
    printf(
        ...
        "rel_presidual = %11.4g; feasibility_tol = %11.4g: primal_feasible = "
        "%d\n",
        presidual_, bounds_measure, rel_presidual, feasibility_tol_,
        primal_feasible);
    printf(
        "Iterate::feasible dresidual_ = %11.4g;  costs_measure = %11.4g; "
        "rel_dresidual = %11.4g; feasibility_tol = %11.4g:   dual_feasible = "
        "%d\n",
        dresidual_, costs_measure, rel_dresidual, feasibility_tol_,
        dual_feasible);
  }
  return is_feasible;
}
...
bool Iterate::term_crit_reached() const {
  if (feasible() && optimal()) {
    if (start_crossover_tol_ <= 0.0) return true;
    double pres, dres;
...
void Iterate::ResidualsFromDropping(double* pres, double* dres) const {
  ...
  if (pres) *pres = presmax;
  if (dres) *dres = dresmax;
}
...
void Iterate::ComputeResiduals() const {
  ...
  if (!postprocessed_) {
    for (Int j = 0; j < n + m; j++)
      if (StateOf(j) == State::fixed) rc_[j] = 0.0;
  }
...
void Iterate::ComputeObjectives() const {
  ...
  if (postprocessed_) {
    ...
    for (Int j = 0; j < n + m; j++) {
      if (std::isfinite(lb[j])) dobjective_ += lb[j] * zl_[j];
      if (std::isfinite(ub[j])) dobjective_ -= ub[j] * zu_[j];
    }
  } else {
    ...
    dobjective_ = Dot(b, y_);
    for (Int j = 0; j < n + m; j++) {
      if (has_barrier_lb(j)) dobjective_ += lb[j] * zl_[j];
      if (has_barrier_ub(j)) dobjective_ -= ub[j] * zu_[j];
      if (StateOf(j) == State::fixed)
