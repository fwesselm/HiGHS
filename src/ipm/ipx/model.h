...
class Model {
 public:
  ...
  Int Load(const Control& control, Int num_constr, Int num_var, const Int* Ap,
           const Int* Ai, const double* Ax, const double* rhs,
           const char* constr_type, const double* obj, const double* lbuser,
           const double* ubuser);
  ...
  bool IsDenseColumn(Int j) const { return AI_.entries(j) >= nz_dense_; }
  ...
  Int PresolveIPMStartingPoint(const double* x_user, const double* xl_user,
                               const double* xu_user, const double* slack_user,
                               const double* y_user, const double* zl_user,
                               const double* zu_user, Vector& x_solver,
                               Vector& xl_solver, Vector& xu_solver,
                               Vector& y_solver, Vector& zl_solver,
  ...
  void PostsolveInteriorSolution(
      const Vector& x_solver, const Vector& xl_solver, const Vector& xu_solver,
      const Vector& y_solver, const Vector& zl_solver, const Vector& zu_solver,
      double* x_user, double* xl_user, double* xu_user, double* slack_user,
      double* y_user, double* zl_user, double* zu_user) const;
  ...
  void EvaluateInteriorSolution(const Vector& x_solver, const Vector& xl_solver,
                                const Vector& xu_solver, const Vector& y_solver,
                                const Vector& zl_solver,
                                const Vector& zu_solver, Info* info) const;
  ...
  void PostsolveBasicSolution(const Vector& x_solver, const Vector& y_solver,
  ...
  void EvaluateBasicSolution(const Vector& x_solver, const Vector& y_solver,
                             const Vector& z_solver,
                             const std::vector<Int>& basic_status_solver,
                             Info* info) const;
  ...
  void PostsolveBasis(const std::vector<Int>& basic_status_solver, Int* cbasis,
                      Int* vbasis) const;
 ...
 private:
  ...
  void ScalePoint(Vector& x, Vector& xl, Vector& xu, Vector& slack, Vector& y,
                  Vector& zl, Vector& zu) const;
  ...
  void ScaleBackInteriorSolution(Vector& x, Vector& xl, Vector& xu,
                                 ...
                                 Vector& zu) const;
  void ScaleBackResiduals(Vector& rb, Vector& rc, Vector& rl, Vector& ru) const;
  void ScaleBackBasicSolution(Vector& x, Vector& slack, Vector& y,
                              Vector& z) const;
  void ScaleBackBasis(std::vector<Int>& cbasis, std::vector<Int>& vbasis) const;
  ...
  // assertion will fail.
  void DualizeIPMStartingPoint(const Vector& x_user, const Vector& xl_user,
                               const Vector& xu_user, const Vector& slack_user,
                               const Vector& y_user, const Vector& zl_user,
                               const Vector& zu_user, Vector& x_solver,
                               Vector& xl_solver, Vector& xu_solver,
                               Vector& y_solver, Vector& zl_solver,
                               Vector& zu_solver) const;
  ...
  // scaled user model.
  void DualizeBackInteriorSolution(
      const Vector& x_solver, const Vector& xl_solver, const Vector& xu_solver,
      const Vector& y_solver, const Vector& zl_solver, const Vector& zu_solver,
      Vector& x_user, Vector& xl_user, Vector& xu_user, Vector& slack_user,
      Vector& y_user, Vector& zl_user, Vector& zu_user) const;
  void DualizeBackBasicSolution(const Vector& x_solver, const Vector& y_solver,
                                const Vector& z_solver, Vector& x_user,
                                Vector& slack_user, Vector& y_user,
                                Vector& z_user) const;
  ...
  void CorrectScaledBasicSolution(Vector& x, Vector& slack, Vector& y,
                                  Vector& z, const std::vector<Int> cbasis,
