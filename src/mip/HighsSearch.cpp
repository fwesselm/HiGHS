...
double HighsSearch::checkSol(const std::vector<double>& sol,
  ...
  for (HighsInt i = 0; i != mipsolver.numCol(); ++i) {
    ...
    if (frac(sol[i]) > mipsolver.mipdata_->feastol) {
...
void HighsSearch::setRINSNeighbourhood(const std::vector<double>& basesol,
  ...
  for (HighsInt i = 0; i != mipsolver.numCol(); ++i) {
    ...
    if (frac(basesol[i]) < mipsolver.mipdata_->feastol) {
      double intval = round(basesol[i]);
...
void HighsSearch::setRENSNeighbourhood(const std::vector<double>& lpsol) {
  for (HighsInt i = 0; i != mipsolver.numCol(); ++i) {
    ...
    double downval = floor(lpsol[i], mipsolver.mipdata_->feastol);
    double upval = ceil(lpsol[i], mipsolver.mipdata_->feastol);
...
HighsInt HighsSearch::selectBranchingCandidate(int64_t maxSbIters,
  ...
  while (true) {
    ...
    double upval = ceil(fracval);
    double downval = floor(fracval);
    ...
    auto analyzeSolution = [&](double objdelta,
      ...
      for (HighsInt k = 0; k != numfrac; ++k) {
        ...
        double otherdownval = floor(fracints[k].second);
        double otherupval = ceil(fracints[k].second);
    ...
    if (branchcand != -1) {
      ...
      switch (childselrule) {
        ...
        case ChildSelectionRule::kRootSol: {
          ...
          double downVal = floor(currnode.branching_point);
          double upVal = ceil(currnode.branching_point);
          if (!subrootsol.empty()) {
...
bool HighsSearch::backtrack(bool recoverBasis) {
  ...
  while (true) {
    ...
    if (currnode.branchingdecision.boundtype == HighsBoundType::kLower) {
      currnode.branchingdecision.boundtype = HighsBoundType::kUpper;
      currnode.branchingdecision.boundval =
          floor(currnode.branchingdecision.boundval, -0.5);
    } else {
      currnode.branchingdecision.boundtype = HighsBoundType::kLower;
      currnode.branchingdecision.boundval =
          ceil(currnode.branchingdecision.boundval, -0.5);
    }
