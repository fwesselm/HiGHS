...
void HighsCutGeneration::separateLiftedKnapsackCover() {
  ...
  auto g = [&](double z) {
    ...
    HighsInt h = std::round(hfrac);
    if (h != 0 && calcFrac(hfrac) * std::max(1.0, abar) <= epsilon &&
        h <= cplussize - 1) {
      halfintegral = true;
      coef = 0.5;
    }
...
bool HighsCutGeneration::separateLiftedMixedIntegerCover() {
  ...
  double eta = calcCeil(mudival);
  HighsCDouble r = mu - calcFloor(mudival) * HighsCDouble(al);
  ...
  HighsInt kmin = calcFloor(eta - upperl - 0.5);
  ...
  int64_t kmax = calcFloor(upperl - eta + 0.5);
...
bool HighsCutGeneration::cmirCutGenerationHeuristic(double minEfficacy,
  ...
  double downrhs = static_cast<double>(calcFloor(scalrhs));
  ...
  for (HighsInt j = 0; j != rowlen; ++j) {
    ...
    if (!isintegral[j]) {
      ...
      double downaj = static_cast<double>(calcFloor(scalaj, kHighsTiny));
