...
static inline void lu_list_init(lu_int* flink, lu_int* blink, lu_int nelem,
                                lu_int nlist, lu_int* min_list) {
  lu_int i;
  for (i = 0; i < nelem + nlist; i++) flink[i] = blink[i] = i;
  if (min_list) *min_list = MAX(1, nlist);
}
...
static inline void lu_list_add(lu_int elem, lu_int list, lu_int* flink,
                               lu_int* blink, lu_int nelem, lu_int* min_list) {
  lu_int temp;
  ...
  if (list > 0 && min_list && list < *min_list) *min_list = list;
}
...
static inline void lu_list_remove(lu_int* flink, lu_int* blink, lu_int elem) {
...
static inline void lu_list_move(lu_int elem, lu_int list, lu_int* flink,
                                lu_int* blink, lu_int nelem, lu_int* min_list) {
...
static inline void lu_list_swap(lu_int* flink, lu_int* blink, const lu_int e1,
                                const lu_int e2) {
