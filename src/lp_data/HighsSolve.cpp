...
void assessExcessiveBoundCost(const HighsLogOptions log_options,
  ...
  if (max_finite_col_cost > kExcessivelyLargeCostValue) {
    ...
    HighsInt suggested_user_cost_scale_setting = calcFloor(std::log2(ratio));
    HighsInt suggested_cost_scale_exponent = calcFloor(std::log10(ratio));
    highsLogUser(
  ...
  if (max_finite_col_bound > kExcessivelyLargeBoundValue) {
    ...
    HighsInt suggested_user_bound_scale = calcFloor(std::log2(ratio));
    HighsInt suggested_bound_scale_exponent = calcFloor(std::log10(ratio));
    if (lp.isMip()) {
      highsLogUser(
          log_options, HighsLogType::kWarning,
  ...
  if (max_finite_col_cost > 0 &&
    ...
    double ratio = kExcessivelySmallCostValue /
                   (max_finite_col_cost / user_cost_scale_value);
    HighsInt suggested_user_cost_scale_setting = calcCeil(std::log2(ratio));
    HighsInt suggested_cost_scale_exponent = calcCeil(std::log10(ratio));
    highsLogUser(
        log_options, HighsLogType::kWarning,
  ...
  if (max_finite_col_bound > 0 &&
    ...
    double ratio = kExcessivelySmallBoundValue /
                   (max_finite_col_bound / user_bound_scale_value);
    HighsInt suggested_user_bound_scale = calcCeil(std::log2(ratio));
    HighsInt suggested_bound_scale_exponent = calcCeil(std::log10(ratio));
    if (lp.isMip()) {
      highsLogUser(
          log_options, HighsLogType::kWarning,
